...
class EvalCallback():
    def __init__(self, net, input_shape, class_names, num_classes, val_lines, log_dir, cuda, \
            ...
            letterbox_image=True, MINOVERLAP=0.5, eval_flag=True, period=1, num_frame=5):
    ...
    def get_history_imgs(self, line):
        ...
        return [os.path.join(dir_path,  "%d.%s" % (max(id, 0),file_type)) for id in range(index-self.num_frame+1, index + 1)]
...
#     with open('coco_val_SIATD.txt', encoding='utf-8') as f:
